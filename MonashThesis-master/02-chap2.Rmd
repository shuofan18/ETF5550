...
We will start with the simulation of the original dataset, x and y, with randomly chosed sample sizes, fit a linear model to it regardless and collect the standardized residuals to make plots. In Simchoni's study, instead of actually plotting the data, he rounded all randomly generated numbers to integers and use them as coordinates directly. To gain more freedom with simulation, we will not change the data and will actually plot them using ggplot2 in R. After that we will save all plots to our local computer, and use Keras package to read them back into R as matrix later on. For simplicity, we will sample x randomly from a uniform distribution from -3 to 3 for each model. 
...
p1 <-   ggplot(cls[[1]], aes(x=yhat, y=res, alpha=0.01)) +geom_point()
p2 <-   ggplot(cls[[2]], aes(x=yhat, y=res, alpha=0.01)) +geom_point()
p3 <-   ggplot(cls[[3]], aes(x=yhat, y=res, alpha=0.01)) +geom_point()
p4 <-   ggplot(cls[[4]], aes(x=yhat, y=res, alpha=0.01)) +geom_point()
