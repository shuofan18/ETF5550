...
######## evaluate only on real data (without lineup)   0.6143 accuracy
...
pred_wo_lineup <- model %>% predict_generator(test_generator_turk, steps = 1)

stat_df <- as.tibble(cbind(pred_wo_lineup, test_generator_turk$filenames, test_generator_turk$classes)) %>%
  rename(
    predict_proba = V1,
    filename = V2,
    test_label = V3
...
#txt.file could be found in "/Volumes/5550/thesis"

######### get prediction for each pic (final compare) ##############
...
##############################################################
############# below are done, no need to repeat  ############  
#############################################################

# get names from "file.csv"
...
########## create 70 folders in "thesis" folder (final compare)
...
########## save 20 images to each folder (final compare)
...
############ create "linear" and "norela" for each folder/70 folders
...
########## copy the real plot into "linear" and the rest to "norela" for all 70 folders
...
############ below is for a separate test (without lineup specification) #######

########### select 'real plots' (70) to a separate folder "evaluate_turk"
...
# move them into 'linear' folder in 'evaluate_turk' and copy 3 no-rela to 'norela' manually
